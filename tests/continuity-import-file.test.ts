...
const VALID_IMPORT_FILE = `# CONTINUITYOS IMPORT FILE
version: 1
source_ai: ChatGPT
...
describe("continuity import file", () => {
  ...
  it("handles missing sections with UNKNOWN defaults", () => {
    const parsed = previewContinuityImportFile(`# CONTINUITYOS IMPORT FILE
version: 1
source_ai: Claude
`);
    expect(parsed.valid).toBe(true);
  ...
  it("includes imported project state in later context packs", () => {
    ...
    expect(pack.text).toContain("## Project State");
  ...
  it("ships the copyable import prompt with the required format", () => {
    expect(CONTINUITY_IMPORT_FILE_PROMPT).toContain("Generate a ContinuityOS Import File");
    expect(CONTINUITY_IMPORT_FILE_PROMPT).toContain("Return only the import file in markdown.");
    expect(CONTINUITY_IMPORT_FILE_PROMPT).toContain("source_ai:");
